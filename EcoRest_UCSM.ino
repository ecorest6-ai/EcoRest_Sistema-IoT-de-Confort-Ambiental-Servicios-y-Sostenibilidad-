...
// =====================================================
// EcoRest UCSM - Firmware de control del modulo
// Sensado de presencia, clima, ruido y actuadores
// =====================================================
...
// =====================================================
// FUNCIONES DE CONTROL BASICO (MATRIZ Y RELES)
// =====================================================
...
void actualizarAudio() {
  ...
  // Si pasa de "despejado" a "presencia", reproduce de inmediato (sin cooldown)
  if (presencia && !presencia_anterior) {
    reproducirTrack(TRACK_PRESENCIA);
  }

  // El aviso de ruido alto sí usa cooldown, para no repetirse en exceso
...
void mostrarSerial() {
  ...
  Serial.printf(" LUZ (Lux) : %.1f \t| RUIDO (pp): %d %s\n", lux, ruido_pp, ruido_alto ? "[ALTO]" : "");
  Serial.println("--------------------------------------------------");
  Serial.printf(" Vent: [%s] | Nebul: [%s] | Matriz: [%s]\n",
                ventilador_activo?"ON":"OFF", nebulizador_activo?"ON":"OFF", matriz_activa?"ON":"OFF");
  Serial.printf(" Audio:[%s] | Vol: %d | Comandos: '+', '-', 'P'\n",
...
void setup() {
  ...
  Serial.println("\nIniciando sistema EcoRest UCSM...");
